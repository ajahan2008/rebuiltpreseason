...
import frc.robot.subsystems.GlobalLimelight;
...
import frc.robot.Commands.CANRange.GetInRange;
import frc.robot.Commands.CANRange.UseCanrange;
...
public class RobotContainer {
    ...
    public static final GlobalLimelight globalLimelight = new GlobalLimelight();
    ...
    private void configureBindings() {
        ...
        joystick.a().whileTrue(new StopDrivetrain(drivetrain, robotCentric));
        joystick.povUp().onTrue(new GetInRange(canRange, drivetrain, robotCentric, .5).withTimeout(3).unless(() -> canRange.getDistance() < .5).andThen(() -> CommandScheduler.getInstance().cancelAll()));
        joystick.b().whileTrue(new UseCanrange(canRange));
